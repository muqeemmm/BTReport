...
def transition_zone_thickness(t1ce_pth, seg_pth,
                              step_vox = 0.25, max_dist_mm = 40.0,) -> float:
    ...
    # 2) edema intensity band (CHANGE #3: robust MAD-based band)
    ...
    # 3) ray direction = nearest edema voxel (unchanged)
    inv_edema = ~ed_mask
    ...
    return float(np.nanmedian(thicknesses)) if len(thicknesses) else np.nan
...
class ExtractT2FLAIRMismatch:
    ...
    def get_brain_mask(self, t2: NiftiImage, flair: NiftiImage) -> np.ndarray:
        """
        Simple image-derived brain mask.
        """
        t2_mask = np.isfinite(t2.array) & (t2.array != 0)
        flair_mask = np.isfinite(flair.array) & (flair.array != 0)
        brain_mask = t2_mask | flair_mask
        return brain_mask
    ...
    def get_center_and_rim_masks(self, core_mask: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        center_mask = core_mask & (dt >= 0.5 * max_dt)
    ...
    def extract_t2_flair_mismatch(
        ...
        if center_mask.sum() < self.min_center_voxels or rim_mask.sum() < self.min_rim_voxels:
            ...
            # Mismatch degree
            t2_tumour_mean = float(np.mean(t2.array[whole_tumor_mask]))
            flair_tumour_mean = float(np.mean(flair.array[whole_tumor_mask]))
    ...
    def __call__(self, tumorseg_ss: str, t2_path: str, flair_path: str) -> dict:
