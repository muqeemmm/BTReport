"""
Glioma classification via Meta Llama 4 Maverick on Vertex AI (Model-as-a-Service).

This is the Llama-4 counterpart of `classify_glioma_groq.py`. It performs the
...
Chain-of-Thought Confidence Elicitation (CoT CE) prompt, schema and metrics, so
Llama 4 Maverick is directly comparable to the other models in this folder.

WHY THIS LOOKS LIKE THE GROQ SCRIPT, NOT THE GEMINI SCRIPT
---------------------------------------------------------
Llama 4 is NOT available on the Gemini Developer API (the AI-Studio `google.genai`
client used by `classify_glioma_gemini25/31.py`). On Google Cloud it is served
through Vertex AI Model-as-a-Service (MaaS) via an **OpenAI-compatible**
ChatCompletions endpoint. So this script reuses the synchronous request->response
loop and the `openai` client from the Groq script, and only the client wiring
(base URL + auth) changes. Billing is 100% Google Cloud / Vertex AI — no OpenAI
account is involved; "OpenAI-compatible" refers only to the request/response shape.

AUTH DIFFERENCE vs. THE OTHER SCRIPTS
-------------------------------------
The Groq/OpenAI/Gemini scripts authenticate with a static API key from `.env`.
Vertex AI authenticates with a short-lived Google Cloud OAuth access token minted
from Application Default Credentials (ADC). Tokens expire (~1 h), so this script
mints/refreshes the token via google-auth and passes it as the OpenAI `api_key`.
See `docs/llama4_vertex_setup.md` for the one-time prerequisites (enable the
Vertex AI API, accept Meta's licence on the model card, `gcloud auth ...`).

CONFIG comes from .env (or the process env):
    VERTEX_PROJECT_ID = your-gcp-project-id
    VERTEX_LOCATION   = us-east5          # region that serves Llama 4 Maverick MaaS
"""
...
# Vertex AI MaaS model id for Llama 4 Maverick 17B-128E. The OpenAI-compatible
# endpoint expects the publisher-prefixed form "meta/<model>".
MODEL_ID    = "meta/llama-4-maverick-17b-128e-instruct-maas"
MODEL_SLUG  = "llama_4_maverick"     # used in output filenames / metric keys
...
# Default region that serves Llama 4 Maverick MaaS (overridable via .env).
DEFAULT_LOCATION = "us-east5"

# Llama Guard is ON by default on Vertex MaaS and may refuse/flag clinical text.
# Set to False to disable input/output moderation (sent via extra_body).
...
# so we constrain generation to glioma_classification_schema.json exactly like the
# gpt55/groq scripts — this prevents the key-drift/collapsed-block failures that
# plain "json_object" mode allowed. Modes: "json_schema" | "json_object" | "none".
...
# Vertex Llama sometimes collapses the three tasks into a single block (a scalar
# `prediction` answering only IDH). It isn't deterministic, so re-asking — with a
# reinforcement nudge and a little temperature — usually fixes it. How many times
# to re-ask before giving up on a subject and skipping it (so a later run retries).
STRUCTURE_RETRIES = 5
...
def parse_model_json(content: str) -> dict:
    """
    Parse the model's JSON. Llama on the Vertex OpenAI endpoint does not guarantee
    strict json_schema enforcement the way OpenAI/Groq do, so be defensive: strip
    any accidental code fences and extract the outermost JSON object if needed.
    """
    text = content.strip()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Classify gliomas via Meta Llama 4 Maverick on Vertex AI "
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = HERE / f"llama4_outputs_{json_type}"
    ...
    print(f"{C}Vertex AI MaaS  |  project={W}{project}{C}  region={W}{location}{C}\n"
          ...
          f"  |  Llama Guard: {W}{'on' if ENABLE_LLAMA_GUARD else 'off'}")
    ...
    # Optional Llama Guard toggle, passed through the OpenAI client as extra_body.
    extra_body = None if ENABLE_LLAMA_GUARD else {
    ...
    # run if the endpoint rejects it (e.g. a reasoning model that lacks json_schema).
        ...
        def _request(messages, temperature):
            ...
            for attempt in range(1, MAX_RETRIES + 1):
                try:
                    ...
                    msg = str(e).lower()
                    # If the endpoint rejected structured output, disable it for the
                    # rest of the run and retry (the prompt + parser still apply).
                    if rf_state["value"] is not None and (
                            "response_format" in msg or "json_schema" in msg
                            or "schema" in msg or "structured" in msg):
                        tqdm.write(f"  {Y}NOTE{Style.RESET_ALL}  endpoint rejected "
                                   f"structured output — disabling it for this run "
                                   f"and retrying.")
                        rf_state["value"] = None
                        continue
                    tqdm.write(f"  {R}ERROR{Style.RESET_ALL}  bad request "
                               f"(possible Llama Guard block): {e}")
                    return None
            return None
