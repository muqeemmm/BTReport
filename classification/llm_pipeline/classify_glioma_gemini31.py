"""
Glioma classification via the Gemini Batch API (gemini-3.1-pro by default).

This is the Gemini-3 counterpart of `classify_glioma_gemini25.py` (which targets
gemini-2.5-pro). It performs the SAME task — joint prediction of IDH status,
1p/19q co-deletion and CNS WHO grade from structured pre-operative MRI metadata,
under WHO CNS5 (2021) — using the SAME Chain-of-Thought Confidence Elicitation
(CoT CE) schema, but routes inference through Gemini's **asynchronous Batch API**
to get the documented 50% cost discount versus synchronous calls.

WHY A SEPARATE SCRIPT FOR GEMINI 3.1 PRO
----------------------------------------
Gemini 2.5 Pro is now a generation behind. Gemini 3.1 Pro is Google's latest
reasoning-first model, with adaptive ("dynamic") thinking, a 1M-token context
window, and integrated grounding (currently in preview). Migrating from the 2.5
line to the 3.x line brings significant improvements in high-level reasoning and
complex instruction following — both of which matter for molecular subtyping
from sparse MRI metadata. (Google also ships Gemini 3 Flash, a preview model
combining Gemini 3 Pro's reasoning with the Flash line's latency/cost profile,
and Gemini 2.5 Flash with a controllable thinking budget; either can be selected
here via --model, but the default targets 3.1 Pro.)

THE ONE DESIGN NOTE THAT CHANGES THE CLIENT/CONFIG
--------------------------------------------------
Gemini 3 models use "dynamic thinking" by DEFAULT, and you control depth via a
`thinking_level` parameter. The older `thinking_budget` (a token count) is
REPLACED by this categorical level on the 3.x line. For a controlled benchmark
we must pin that parameter the same way we pin `temperature=0.0`, otherwise the
amount of reasoning the model spends varies run-to-run and the comparison is no
longer apples-to-apples. We therefore fix `thinking_level` to a single value
(see THINKING_LEVEL) for every request. Everything else — prompt, schema, batch
mechanics, metrics — is intentionally identical to the 2.5 Pro script so the two
...
# Gemini 3 uses categorical "dynamic thinking" controlled by `thinking_level`
# (this REPLACES the 2.5-era `thinking_budget` token count). Valid levels:
# minimal | low | medium | high (the SDK coerces lowercase to the ThinkingLevel
# enum). Pin it for a controlled benchmark, exactly like temperature=0.0. "high"
# matches this reasoning-first molecular-subtyping task; override --thinking-level.
THINKING_LEVEL   = "high"
...
def model_slug(model: str) -> str:
    """gemini-3.1-pro -> gemini_3_1_pro (for filenames / keys)."""
...
def build_request_config(thinking_level: str = THINKING_LEVEL) -> dict:
    """
    Per-request config dict for an inline batch request (CoT-CE structured output).

    Gemini 3 difference vs. the 2.5 Pro script: thinking is now controlled by the
    categorical `thinking_level` (passed via ThinkingConfig), which REPLACES the
    2.5-era `thinking_budget` token count. We pin it to a single level so the
    amount of reasoning is held constant across the benchmark — the same reason
    we pin temperature=0.0 and seed=42.
    """
    return {
...
def phase_submit(client, output_dir, json_type, model, id_map, force,
                 thinking_level, C, G, Y, W, D):
    ...
    print(f"{C}Submitting {W}{len(pending)}{C} subjects in chunks of {CHUNK_SIZE} "
          f"using {W}{model}{C} (thinking_level={W}{thinking_level}{C}, "
          f"batch = 50% off).{Style.RESET_ALL}")

    state = load_state(output_dir)
    req_cfg = build_request_config(thinking_level)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Classify gliomas via the Gemini Batch API (gemini-3.1-pro, "
                    "50% cheaper).")
    ...
    p.add_argument("--thinking-level", default=THINKING_LEVEL,
                   choices=["minimal", "low", "medium", "high"],
                   help=f"Gemini 3 dynamic-thinking depth (ThinkingLevel), pinned "
                        f"for a controlled benchmark (default {THINKING_LEVEL}). "
                        f"Replaces the 2.5-era thinking_budget token count.")
...
def main() -> None:
    ...
    output_dir = HERE / f"gemini31_outputs_{args.json_type}" if args.json_type \
        else HERE / "gemini31_outputs"
    ...
    if args.phase == "submit":
        phase_submit(client, output_dir, args.json_type, args.model, id_map,
                     args.force, args.thinking_level, C, G, Y, W, D)
