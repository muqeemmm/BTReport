"""
Glioma classification via OpenBioLLM-70B (Saama AI Labs) (Llama-3 70B fine-tuned for the biomedical domain)
served through an OpenAI-compatible chat-completions API.

This is the OpenBioLLM-70B (Saama AI Labs) counterpart of `classify_glioma_deepseek.py`. It performs
...
structure-retry / content-based extraction machinery, so OpenBioLLM-70B (Saama AI Labs) is directly
...
2) deepinfra    -- these four medical models are NOT in DeepInfra's serverless
   catalog, so you must first create a **Custom LLM deployment** (one-time):
     ...
     b. Point it at the Hugging Face repo `aaditya/Llama3-OpenBioLLM-70B`, pick a GPU
...
OpenBioLLM-70B is a standard (non-reasoning) instruct model: it answers
...
MODEL_SLUG  = "openbiollm_70b"            # used in output filenames / metric keys
...
PROVIDERS = {
    ...
    "deepinfra": {
        ...
        "model_id":      "aaditya/Llama3-OpenBioLLM-70B",  # HF repo to deploy (not callable as-is)
        "needs_deploy":  True,                      # requires a Custom-LLM deployment id
        "deploy_repo":   "aaditya/Llama3-OpenBioLLM-70B",
    },
}
DEFAULT_PROVIDER = "featherless"

# OpenBioLLM-70B (Saama AI Labs) exposes no Llama-Guard-style per-request safety toggle, so
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Classify gliomas via OpenBioLLM-70B (Saama AI Labs) on an OpenAI-compatible "
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = HERE / f"openbiollm_outputs_{json_type}"
