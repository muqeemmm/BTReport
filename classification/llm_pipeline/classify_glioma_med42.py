"""
Glioma classification via Med42-v2-70B (M42) (Llama-3 70B clinical model, instruct + preference tuned)
served through an OpenAI-compatible chat-completions API.

This is the Med42-v2-70B (M42) counterpart of `classify_glioma_deepseek.py`. It performs
...
structure-retry / content-based extraction machinery, so Med42-v2-70B (M42) is directly
...
2) deepinfra    -- these four medical models are NOT in DeepInfra's serverless
   catalog, so you must first create a **Custom LLM deployment** (one-time):
     ...
     b. Point it at the Hugging Face repo `m42-health/Llama3-Med42-70B`, pick a GPU
...
Med42-v2-70B is a standard (non-reasoning) instruct model: it answers
...
MODEL_SLUG  = "med42_v2_70b"            # used in output filenames / metric keys
...
PROVIDERS = {
    ...
    "deepinfra": {
        ...
        "model_id":      "m42-health/Llama3-Med42-70B",  # HF repo to deploy (not callable as-is)
        "needs_deploy":  True,                      # requires a Custom-LLM deployment id
        "deploy_repo":   "m42-health/Llama3-Med42-70B",
    },
}
DEFAULT_PROVIDER = "featherless"

# Med42-v2-70B (M42) exposes no Llama-Guard-style per-request safety toggle, so
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Classify gliomas via Med42-v2-70B (M42) on an OpenAI-compatible "
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = HERE / f"med42_outputs_{json_type}"
