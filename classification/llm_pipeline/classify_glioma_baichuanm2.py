"""
Glioma classification via Baichuan-M2-32B (Qwen2.5-32B-based medical REASONING model w/ verifier system)
served through an OpenAI-compatible chat-completions API.

This is the Baichuan-M2-32B counterpart of `classify_glioma_deepseek.py`. It performs
...
structure-retry / content-based extraction machinery, so Baichuan-M2-32B is directly
...
2) deepinfra    -- these four medical models are NOT in DeepInfra's serverless
   catalog, so you must first create a **Custom LLM deployment** (one-time):
     ...
     b. Point it at the Hugging Face repo `baichuan-inc/Baichuan-M2-32B`, pick a GPU
...
Baichuan-M2-32B is a REASONING model: it emits a <think>...</think> chain of
thought before the JSON answer. MAX_OUTPUT_TOKENS is raised to 8192 to leave
headroom, and parse_model_json() strips the think block before parsing.
"""
...
MODEL_SLUG  = "baichuan_m2_32b"            # used in output filenames / metric keys
...
PROVIDERS = {
    ...
    "deepinfra": {
        ...
        "model_id":      "baichuan-inc/Baichuan-M2-32B",  # HF repo to deploy (not callable as-is)
        "needs_deploy":  True,                      # requires a Custom-LLM deployment id
        "deploy_repo":   "baichuan-inc/Baichuan-M2-32B",
    },
}
DEFAULT_PROVIDER = "featherless"

# Baichuan-M2-32B exposes no Llama-Guard-style per-request safety toggle, so
# nothing extra is sent in the request body.
ENABLE_LLAMA_GUARD = False

# This model IS a reasoning model (emits a <think> block). Reasoning tokens are emitted BEFORE the
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Classify gliomas via Baichuan-M2-32B on an OpenAI-compatible "
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = HERE / f"baichuanm2_outputs_{json_type}"
