"""
Glioma classification via DeepSeek on Vertex AI — BATCH prediction (50% cheaper).

This is the batch counterpart of `classify_glioma_deepseek.py` (synchronous) and
the DeepSeek sibling of `classify_glioma_llama4_batch.py`. It performs the SAME
task — joint IDH / 1p19q / WHO-grade prediction from structured MRI metadata under
WHO CNS5 (2021) — with the SAME prompt, schema, metrics and content-based
extraction, but routes inference through a Vertex AI **batchPredictionJob** for the
documented 50% discount versus the synchronous endpoint.

WHY IT'S NEARLY IDENTICAL TO THE LLAMA 4 BATCH SCRIPT
-----------------------------------------------------
Vertex AI batch consumes the **OpenAI batch JSONL schema** — each line is
{"custom_id", "method", "url": "/v1/chat/completions", "body": {...}}. The
three-phase submit -> poll -> fetch flow, GCS staging and `custom_id` matching are
reused verbatim. Only the DeepSeek specifics differ:

  1. MODEL_ID / PUBLISHER_MODEL use the `deepseek-ai/...` publisher.
  2. The serving region differs (DeepSeek R1/V3.1 -> us-central1). NOTE: batch jobs
     are REGIONAL resources, so VERTEX_LOCATION must be a region — the `global`
     endpoint is online-only and is rejected here for batch.
  3. DeepSeek is a reasoning model: outputs may carry a <think>...</think> block,
     which parse_model_json() strips before JSON parsing.

Transport (unchanged from the Llama batch script):
    upload JSONL to a Cloud Storage bucket (gs://)
    POST .../batchPredictionJobs (REST, ADC auth) ; GET to poll
    download the output JSONL from gs://
...
CONFIG (from .env or the process env):
    ...
    VERTEX_LOCATION   = us-central1                  # REGION serving the model
...
# DeepSeek MaaS ids. Per-line body uses "deepseek-ai/<model>"; the job's `model`
# field uses the publisher resource path. Swap all three together to benchmark
# another DeepSeek model (e.g. deepseek-v3.1-maas, deepseek-v3.2-maas).
MODEL_ID    = "deepseek-ai/deepseek-r1-0528-maas"               # per-line body model
PUBLISHER_MODEL = "publishers/deepseek-ai/models/deepseek-r1-0528-maas"
MODEL_SLUG  = "deepseek_r1_0528"
...
DEFAULT_LOCATION = "us-central1"   # batch is regional; "global" is not valid here
...
# which prevents key-drift/collapsed-block failures. BUT batch can't fall back
# mid-job, and R1 (a reasoning model) MIGHT reject `response_format` — which would
# fail the whole job. So this DEFAULTS TO "none" for the R1 default: validate with
# `classify_glioma_deepseek.py` (the sync script auto-detects support) and, if your
# model accepts json_schema, set this to "json_schema". For Llama / DeepSeek V3.x
# (known to support it) "json_schema" is the right choice.
...
def load_vertex_config() -> tuple[str, str, str]:
    ...
    if location == "global":
        raise RuntimeError("Batch prediction jobs are regional; set VERTEX_LOCATION "
                           "to a region that serves the model (e.g. us-central1), "
                           "not 'global'.")
...
# --- JSON extraction (defensive — Vertex MaaS doesn't guarantee strict schema) -

_FENCE_RE = re.compile(r"^```(?:json)?\s*|\s*```$", re.MULTILINE)
# DeepSeek V3.1/R1 may prepend a <think>...</think> reasoning block; strip it.
...
def build_request_line(folder_name: str, prompt: str) -> dict:
    ...
    body = {
        ...
        "temperature": 0.0,
        "seed": SEED,
        # DeepSeek reasoning (<think>) consumes output tokens before the JSON,
        # so allow generous headroom to avoid truncating the answer.
        "max_tokens": 8192,
    }
    # Strict structured output if RESPONSE_FORMAT_MODE enables it (off by default
    # for R1 — see the constant). The content-based extractor + <think> stripping
    # remain a safety net at read time regardless.
...
def phase_submit(session, gcs, bucket, api, output_dir, json_type, id_map,
    ...
    for ci, chunk in enumerate(chunked(pending, CHUNK_SIZE), 1):
        ...
        base       = f"deepseek_batch/{json_type}/{stamp}/chunk{ci}"
...
def _extract_from_line(line: dict):
    ...
    DeepSeek reasoning models may return content=null with the answer in
    `reasoning_content`; fall back to it.
    """
...
def parse_args() -> argparse.Namespace:
    ...
    # Required for every phase: outputs and batch_state.json live in a per-json-type
    # directory, so we need it to locate the right folder.
    p.add_argument("--json-type", choices=["btreport", "btreport_pp"], required=True,
                   help="Which metadata JSON variant to use.")
...
def main() -> None:
    ...
    output_dir = HERE / f"deepseek_batch_outputs_{args.json_type}"
    output_dir.mkdir(parents=True, exist_ok=True)

    # `metrics` is purely local scoring — no Vertex/GCS credentials needed.
    ...
    gt     = load_ground_truth(COHORT_XLSX) if args.phase == "run" else {}
